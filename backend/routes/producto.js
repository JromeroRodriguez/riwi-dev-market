...
router.post("/", requiereRol("vendedor", "administrador"), async (req, res) => {
  try {
    const { titulo, descripcion, categoria_id, precio, url_repositorio } = req.body || {};
    const campos_obligatorios = ["titulo", "descripcion", "categoria_id", "precio", "url_repositorio"];
...
router.put("/:producto_id", requiereRol("vendedor", "administrador"), async (req, res) => {
  try {
    if (!(await productoModel.es_propietario(req.params.producto_id, req.usuario.id))) {
      return res.status(403).json({ error: "No tienes permiso para editar este producto" });
    }
    const producto = await productoModel.actualizar_producto(req.params.producto_id, req.usuario.id, req.body || {});
    if (!producto) return res.status(400).json({ error: "No se enviaron campos válidos para actualizar" });
