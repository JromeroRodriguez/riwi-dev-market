...
async function promedio_calificacion_vendedor(vendedor_id) {
  const { rows } = await pool.query(
    `SELECT COALESCE(AVG(cal.puntuacion), 0) AS promedio, COUNT(cal.id) AS total
     FROM calificaciones cal
     JOIN compras co ON co.id = cal.compra_id
...
module.exports = {
  ...
  crear_calificacion,
  listar_por_producto,
  promedio_calificacion_vendedor,
};
