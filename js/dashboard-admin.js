...
async function cargarPendientes() {
  ...
  try {
    ...
    if (!data.productos.length) {
      contenedor.innerHTML = `<p class="vacio">No hay productos pendientes de revisión. 🎉</p>`;
      return;
    }
...
async function cargarUsuarios() {
  ...
  try {
    ...
    contenedor.innerHTML = data.usuarios.map((u) => `
      <div class="lista-item">
        <div>
          <p style="font-weight:600;margin:0">${u.nombre} <span class="badge ${u.estado === 'bloqueado' ? 'rechazado' : 'publicado'}" style="margin-left:6px">${u.estado}</span></p>
        ...
        <div class="acciones">
          ${u.estado === "bloqueado"
            ...
            : `<button class="secundario peligro" data-action="bloquear-usuario" data-usuario-id="${u.id}">Bloquear</button>`}
        </div>
      </div>
    `).join("");
