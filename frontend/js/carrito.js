(function () {
  ...
  async function renderCarrito() {
    ...
    carrito.forEach((item) => {
      ...
      fila.innerHTML = `
        ...
        <button class="carrito-item-eliminar" data-id="${item.producto_id}" title="Eliminar del carrito">✕</button>
      `;
  ...
  async function comprarCarrito() {
    ...
    const confirmado = await alertaConfirmar(
      `Se procesarán ${seleccionados.length} compra(s) por un total de ${app.api.formatoMoneda(totalCarrito())}.`,
      "¿Confirmar compra?"
    );
